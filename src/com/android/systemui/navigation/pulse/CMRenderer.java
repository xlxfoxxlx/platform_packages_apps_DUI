...
public class CMRenderer extends Renderer implements ColorAnimator.ColorAnimationListener {
    ...
    public CMRenderer(Context context, Handler handler, PulseObserver callback) {
    ...
    public void setLeftInLandscape(boolean leftInLandscape) {
        if (mLeftInLandscape != leftInLandscape) {
            ...
            onSizeChanged(mCallback.getWidth(), mCallback.getHeight(), 0, 0);
        }
    }

    private void toggleLavaLamp() {
    ...
    private void setPortraitPoints(int width, int height) {
        float barUnit = width / 32f;
        ...
        for (int i = 0; i < 32; i++) {
            ...
            mFFTPoints[i * 4 + 3] = height;
        }
    }

    private void setVerticalPoints(int width, int height) {
        float barUnit = height / 32f;
        ...
        for (int i = 0; i < 32; i++) {
            ...
            mFFTPoints[i * 4] = mLeftInLandscape ? 0 : width;
            mFFTPoints[i * 4 + 2] = mLeftInLandscape ? 0 : width;
        }
    }

    @Override
    public void onSizeChanged(int w, int h, int oldw, int oldh) {
        final int width = mCallback.getWidth();
        final int height = mCallback.getHeight();
        mVertical = height > width;
        loadValueAnimators();
        if (mVertical) {
            setVerticalPoints(width, height);
        } else {
            setPortraitPoints(width, height);
        }
    }
